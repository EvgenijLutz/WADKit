...
GRAPHICS_DEVICE* graphicsDeviceCreateDefaultMetalDevice(void)
{
	...
	arrayInitialize(&device->commandQueues, sizeof(COMMAND_QUEUE));
	...
	device->createTexture2dWithDataFunc = _metal_createTexture2dWithData;
...
void graphicsDeviceReleaseDefaultMetalDevice(GRAPHICS_DEVICE* graphicsDevice)
{
	...
	assert(graphicsDevice->commandQueues.length == 0);
	arrayDeinitialize(&graphicsDevice->commandQueues);
